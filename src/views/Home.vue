        ...
        <div class="panel-heading">
          <ul class="list-inline topic-filter">
            <li v-for="item in filters">
              <router-link v-title="item.title" :class="{ article: filter === item.filter }" :to="`/topics?filter=${item.filter}`">{{ item.name }}</router-link>
            </li>
          </ul>
        ...
        <div class="panel-body remove-padding-horizontal">
          <ul class="list-group row topic-list">
            <li v-for="article in articles" :key="article.articleId" class="list-group-item">
              <router-link :to="`/articles/${article.articleId}`" tag="div" class="reply_count_area hidden-xs pull-right">
                <div class="count_set">
...
<script>
  ...
  export default {
      ...
      created() {
          this.articles = this.$store.getters.computedArticles()
      },
      computed: {
          ...mapState([
              'auth',
